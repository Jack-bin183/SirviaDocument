...
~~~java
@ImportResource({"classpath*:adminApplicationContext.xml"}, {"classpath*:springMVC-servlet.xml"})
~~~
...
在当前的项目中准备一个类，在类的`main`方法中执行`Generate`的`generateUserTable()`方法来创建用户表。
...
创建用户表后就需要创建账户了，还是在该类的`main`方法中执行`Generate`的`createUser()`方法。
